...
AS_instrList AS_rewriteSpill(F_frame f, AS_instrList il, Temp_tempList spills)
{
  for (; spills; spills = spills->tail)
  {
    ...
    for (; i; i = i->tail)
    {
      ...
      if (Temp_inTempList(t, src))
      {
        Temp_temp newTemp = Temp_newtemp();

        char a[256];
        sprintf(a, "\tmovq %d(`s0), `d0 #spill", acc->u.offset);
        AS_instr newInstr = AS_Oper(String(a), Temp_TempList(newTemp, NULL), Temp_TempList(F_FP(), NULL), NULL);
      ...
      if (Temp_inTempList(t, dst))
      {
        Temp_temp newTemp = Temp_newtemp();
        char buf[256];
        sprintf(buf, "\tmovq `s0, %d(`s1) #spill", acc->u.offset);
