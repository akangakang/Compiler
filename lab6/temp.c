...
Temp_tempList Temp_union(Temp_tempList a, Temp_tempList b) {
	...
	for(Temp_tempList temps1 = a; temps1; temps1 = temps1->tail) {
		res = Temp_TempList(temps1->head, res);
	}

	for(Temp_tempList temps2 = b; temps2; temps2 = temps2->tail) {
		bool found = FALSE;
		for(Temp_tempList temps1 = a; temps1; temps1 = temps1->tail) {
...
// 在a里 但不在b里的
Temp_tempList Temp_difference(Temp_tempList a, Temp_tempList b) {
	Temp_tempList res = NULL;
	for(Temp_tempList temps1 = a; temps1; temps1 = temps1->tail) {
		bool found = FALSE;
		for(Temp_tempList temps2 = b; temps2; temps2 = temps2->tail) {
...
bool Temp_equalTempList(Temp_tempList a, Temp_tempList b) {
	for(Temp_tempList temps1 = a; temps1; temps1 = temps1->tail) {
		bool found = FALSE;
		for(Temp_tempList temps2 = b; temps2; temps2 = temps2->tail) {
			if(temps1->head == temps2->head) {
