...
struct expty transExp(S_table venv, S_table tenv, A_exp a, Tr_level level, Temp_label label)
{
  ...
  case A_callExp:
  {
    ...
    E_enventry func = S_look(venv, get_callexp_func(a));
		...
		if (formals != NULL)
		{
			...
			return expTy(Tr_callExp(level,func->u.fun.level, func->u.fun.label, params), actual_ty(get_func_res(func)));
		}
  ...
  case A_seqExp:
  {
    ...
    Tr_expList eseq = NULL;
    // EM_error(0, "A_seqExp");
    for (A_expList exps = a->u.seq; exps != NULL && exps->head; exps = exps->tail)
    {
